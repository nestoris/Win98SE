...
cmd = "grep -Eoi ^Context=.*$ "themesbash"|cut -d= -f2|sort|uniq|xargs" #команда чтения контекстов из файлов тем
#print themesbash
while ((cmd|getline conts_bash)>0);
sub(/\n$/,"",conts_bash) #удаление переводов строк в конце и создание одной строки с контекстами из файла(ов) тем(ы) (пробел уже есть)
